...
public class EvolveFromPokemonQuest extends PixelmonQuestContainer {
    public EvolveFromPokemonQuest(JavaPlugin javaPlugin) {
        super(javaPlugin);
    }

    @EventHandler
    public void onEvolve(PixelmonEvent pixelmonEvent) {
        if(pixelmonEvent.getPixelmonEvent() instanceof EvolveEvent.Pre) {
            EvolveEvent.Pre event = (EvolveEvent.Pre) pixelmonEvent.getPixelmonEvent();
            try {
                Player player = Bukkit.getPlayer(event.getPlayer().getUUID());
                if (player != null) {
                    Pokemon pokemon = event.getPokemon();
                    this.executionBuilder("evolve-from")
                            .player(player)
                            .root(pokemon)
                            .progressSingle()
                            .buildAndExecute();
